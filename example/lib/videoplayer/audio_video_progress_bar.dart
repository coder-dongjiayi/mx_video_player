...
class ProgressBar extends LeafRenderObjectWidget {
  ...
  _RenderProgressBar createRenderObject(BuildContext context) {
    ...
    return _RenderProgressBar(
      ...
      segmentedDurationList: segmentedDurationList
    );
  }
...
class _RenderProgressBar extends RenderBox {
  _RenderProgressBar({
    ...
    required TimeLabelType timeLabelType,
     List<Duration>? segmentedDurationList,
    ...
    double timeLabelPadding = 0.0,
        ...
        _segmentedDurationList = segmentedDurationList??[],
  ...
  List<Duration> _segmentedDurationList;

  set segmentedDurationList(List<Duration> value) {
    if(_segmentedDurationList == value) return;
    _segmentedDurationList = value;
    markNeedsLayout();
  }

  List<Duration> get segmentedDurationList => _segmentedDurationList;
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    _drawSegmentedPoint(canvas);
    canvas.restore();
  }

  /// TODO:画进度条上的点
  void _drawSegmentedPoint(Canvas canvas){

  if(segmentedDurationList.length == 0) return;
   double width = size.width;
   int totalMilliseconds = _total.inMilliseconds;
   double radius = 6;
   double _dy = 10;

   for(int index = 0; index < segmentedDurationList.length;index ++){
     Paint paint  =  Paint();
     paint.color = Color.fromRGBO(255, 255, 255, 0.6);
     paint.strokeWidth = 1;
     paint.style = PaintingStyle.fill;
     Duration  element =  segmentedDurationList[index];
     int inMilliseconds = element.inMilliseconds;
     double _dx = width * (inMilliseconds/totalMilliseconds)+radius/2;
     canvas.drawCircle(Offset(_dx, _dy), radius, paint);
   }
   canvas.save();
